...
const NotStartedListing = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                setLoading(true)
                const res = await fetch(`/api/projects/search?etat=not started`)
            ...
            >
                <div className="content">
                    <h2 className='text-3xl font-bold sm:text-5xl font-heading  text-[#3A5A40] sm:text-left '>
                        Découvrir nos futures Projets!
                    </h2>
                    <p className='font-content font-medium text-sm sm:text-lg mt-3 max-w-3xl'>
                    Découvrez l'avenir de l'immobilier avec notre collection exclusive de projets à venir. Plongez dans l'innovation et l'excellence architecturale qui redéfiniront votre expérience résidentielle. Chaque projet incarne notre engagement à créer des espaces de vie modernes et inspirants, adaptés aux besoins et aux aspirations de demain.                    </p>
                </div>
                ...
                <div className="btn_container flex items-center justify-center">
                    <button
                       ...
                       onClick={()=>navigate('/searchProject?filter=not started')}
                    >
...
export default NotStartedListing